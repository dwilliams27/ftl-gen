...
@app.command()
def diagnose(
    mod_name: Annotated[str, typer.Argument(help="Mod name or path to mod")],
    launch: Annotated[bool, typer.Option("--launch", help="Patch and launch with monitoring")] = False,
):
    """Run diagnostic checks on a mod.

    Checks for event loops, dangling references, missing required fields,
    and known crash patterns.

    Examples:
        ftl-gen diagnose "Echoes of Hyrule"
        ftl-gen diagnose "Echoes of Hyrule" --launch
    """
    settings = get_settings()

    # Find mod directory
    mod_dir = settings.output_dir / mod_name
    ftl_path = settings.output_dir / f"{mod_name}.ftl"

    if not mod_dir.exists() and not ftl_path.exists():
        console.print(f"[red]Mod not found: {mod_name}[/]")
        raise typer.Exit(1)

    console.print(f"\n[bold]Diagnostic Report: {mod_name}[/]")
    console.print("─" * 50)
    ...
    if mod_dir.exists():
        ...
        if result.ok:
            console.print("  XML Syntax          [green]PASS[/]")
        else:
            console.print(f"  XML Syntax          [red]FAIL[/]  {'; '.join(result.errors)}")
            all_pass = False
    else:
        console.print("  XML Syntax          [yellow]SKIP[/]  No mod directory")
    ...
    if data_dir.exists():
        ev_file = data_dir / "events.xml.append"
        bp_file = data_dir / "blueprints.xml.append"
        if ev_file.exists():
    ...
    if blueprints_xml:
        try:
            ...
            if field_errors:
                console.print(f"  Required Fields     [red]FAIL[/]  {'; '.join(field_errors)}")
                all_pass = False
            else:
                console.print("  Required Fields     [green]PASS[/]")
        except etree.XMLSyntaxError:
            console.print("  Required Fields     [red]FAIL[/]  XML parse error")
            all_pass = False
    else:
        console.print("  Required Fields     [green]PASS[/]")
    ...
    if events_xml:
        ...
        if cycles:
            ...
            console.print(f"  Event Loops         [red]FAIL[/]  {'; '.join(cycle_strs)}")
            all_pass = False
        else:
            console.print("  Event Loops         [green]PASS[/]")
    else:
        console.print("  Event Loops         [green]PASS[/]")
    ...
    if events_xml:
        ...
        if dangling:
            console.print(f"  Dangling Refs       [yellow]WARN[/]  {'; '.join(dangling)}")
        else:
            console.print("  Dangling Refs       [green]PASS[/]")
    else:
        console.print("  Dangling Refs       [green]PASS[/]")
    ...
    if crash_issues:
        console.print(f"  Crash Patterns      [red]FAIL[/]  {'; '.join(crash_issues)}")
        all_pass = False
    else:
        console.print("  Crash Patterns      [green]PASS[/]")

    console.print("─" * 50)
    if all_pass:
        console.print("[green bold]All checks passed[/]")
    else:
        console.print("[red bold]Issues found[/]")

    # Optional: patch and launch with monitoring
    if launch:
        if not ftl_path.exists():
            console.print(f"\n[red]No .ftl file found for launch: {ftl_path}[/]")
            raise typer.Exit(1)

        console.print(f"\n[bold]Launching with monitoring...[/]")
        slipstream = SlipstreamManager(settings)
        if not slipstream.is_available():
            console.print("[red]Slipstream not available[/]")
            raise typer.Exit(1)

        patch_result, launch_result = slipstream.patch_and_launch(
            [ftl_path], mod_name=mod_name
        )

        if not patch_result.success:
            console.print(f"[red]Patch failed: {patch_result.message}[/]")
            raise typer.Exit(1)

        console.print(f"[green]Mod patched[/]")

        if launch_result is None:
            console.print("[red]Launch was not attempted[/]")
            raise typer.Exit(1)

        if launch_result.hang_detected:
            console.print(f"[red bold]HANG DETECTED:[/] {launch_result.message}")
        elif launch_result.success:
            console.print(f"[green]{launch_result.message}[/]")
        else:
            console.print(f"[red]{launch_result.message}[/]")

        if launch_result.errors_in_log:
            console.print("\n[red bold]Errors in FTL.log:[/]")
            for line in launch_result.errors_in_log:
                console.print(f"  [red]{line}[/]")

        if launch_result.log_tail:
            console.print("\n[dim]FTL.log tail:[/]")
            for line in launch_result.log_tail[-20:]:
